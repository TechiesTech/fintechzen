  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      <nav className="max-w-full mx-auto flex items-center justify-between py-4 px-12 backdrop-blur-xl bg-white/10 border border-white/20 shadow-2xl rounded-2xl bg-gradient-to-r from-white/20 via-white/10 to-white/20 hover:shadow-3xl transition-all duration-500 ease-out">
        ...
        {/* Logo */}
        <a href="/" className="relative z-10 text-2xl font-bold ml-12 group cursor-pointer">
          ...
          {navLinks.map((link, index) => (
            link.isScroll ? (
              ...
              <a
                key={link.href}
                href={link.href}
                className="relative font-medium transition-all duration-300 group text-slate-700 hover:text-teal-800 transform hover:scale-105 hover:drop-shadow-lg"
              >
                <span className="relative z-10">{link.name}</span>
                <div className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-teal-500 to-emerald-500 transition-all duration-500 ease-out ${currentPath === link.href ? "w-full" : "w-0 group-hover:w-full"}`}></div>
                <div className="absolute inset-0 bg-gradient-to-r from-teal-400/20 to-emerald-400/20 rounded-lg blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"></div>
              </a>
            )
          ))}
